import streamlit as st
import pickle
from sklearn.feature_extraction.text import CountVectorizer
import pandas as pd
from sklearn.model_selection import train_test_split
from sklearn.feature_extraction.text import CountVectorizer
from sklearn.naive_bayes import MultinomialNB
import pickle


bbc_text = pd.read_csv('bbc-text.txt')
bbc_text=bbc_text.rename(columns = {'text': 'News_Headline'}, inplace = False)
bbc_text.category = bbc_text.category.map({"tech":0,"business":1,'sport':2, 'entertainment':3, 'politics':4})

X=bbc_text.News_Headline
y=bbc_text.category
X_train, X_test, y_train, y_test = train_test_split(X,y,train_size=0.6,random_state=1)


vector = CountVectorizer(stop_words="english",lowercase=False)
vector.fit(X_train)

X_train_transformed = vector.transform(X_train)
X_train_transformed.toarray()
X_test_transformed = vector.transform(X_test)

naivebayes = MultinomialNB()
naivebayes.fit(X_train_transformed, y_train)

saved_model = pickle.dumps(naivebayes)
model=pickle.loads(saved_model)

def classify(headline):
    vec = vector.transform([headline]).toarray()
    return (str(list(model.predict(vec))[0]).replace('0', 'Tech').replace('1', 'Business').replace('2', 'Sports').replace('3','Entertainment').replace('4','Politics'))


st.title('News Classifier')

headline = st.text_input('Enter the news headline')

if st.button('Classify'):
    category = classify(headline)
    st.write(f"The news belongs to '{category}' category.")



